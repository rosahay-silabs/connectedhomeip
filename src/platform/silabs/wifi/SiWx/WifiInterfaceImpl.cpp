...
sl_status_t BackgroundScanCallback(sl_wifi_event_t event, sl_wifi_scan_result_t * result, uint32_t result_length, void * arg)
{
    ...
    if (arg != nullptr)
    {
        sl_wifi_ssid_t * requestedSsidPtr = reinterpret_cast<sl_wifi_ssid_t *>(arg);
        requestedSsidSpan                 = chip::ByteSpan(requestedSsidPtr->value, requestedSsidPtr->length);
        VerifyOrReturnError(!requestedSsidSpan.empty(), SL_STATUS_INVALID_HANDLE);
    }
    ...
    for (uint32_t i = 0; i < nbreResults; i++)
    {
        ...
        if (requestedSsidSpan.empty() || requestedSsidSpan.data_equal(ssidSpan))
        {
            ...
            int16_t rssi = (-1) * result->scan_info[i].rssi_val; // The returned value is positive - we need to flip it
            if (rssi > INT8_MAX)
            {
                rssi = INT8_MAX;
            }
...
CHIP_ERROR WifiInterfaceImpl::GetAccessPointInfo(NetworkCommissioning::WiFiScanResponse & info)
{
    ...
    info.signal.strength = static_cast<int8_t>(rssi);

...
static void dic_task_fn(void * args)
{
    ...
    if (0 !=
        MQTT_Transport_SSLConfigure(transport, (uint8_t *) DIC_SERVER_CA_CERT, strlen(DIC_SERVER_CA_CERT) + 1,
                                    (uint8_t *) DIC_DEVICE_KEY, strlen(DIC_DEVICE_KEY) + 1, NULL, 0, (uint8_t *) DIC_DEVICE_CERT,
                                    strlen(DIC_DEVICE_CERT) + 1))
    {
...
dic_err_t DIC_Init()
{
    ...
    if ((pdPASS != xTaskCreate(dic_task_fn, DIC_TASK_NAME, DIC_TASK_STACK_SIZE, NULL, 1, &dicTask)) || !dicTask)
    {
